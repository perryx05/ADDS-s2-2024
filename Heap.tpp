...
class Heap {
    ...
    while (index > 0) {
        ...
        if (this->tree[index] < this->tree[parentIndex]) {
            std::swap(this->tree[index], this->tree[parentIndex]);
            index = parentIndex;
        } else {
            break;
        }
        ...
        void remove(T value) {
            this -> tree.erase(std::remove(this->tree.begin(), this->tree.end(), value), this->tree.end());
